...
export const fixDescriptionsLogDetails = (info, source) => {
  ...
  return `分组倍率 ${group_ratio}，视频每秒价格 ${symbol}${price}，秒数 ${duration}，分辨率 ${resolution}`;
};
// 修复描述"计费过程"
export const fixDescriptionsCalculateProcess = (info, source) => {
  ...
  return `分组倍率：${group_ratio} * 分辨率(${resolution})价格：${symbol}${price} * 视频秒数：${duration} = ${symbol}${price_total}`;
};